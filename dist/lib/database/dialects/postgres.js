...
class PostgresDialect extends abstract_1.AbstractDialect {
    constructor(sequelize) {
        super(sequelize);
    }
    showTables() {
        let promises = [];
        return this.sequelize.getQueryInterface().showAllTables().then((tables) => {
    ...
    addConstraint(table, constraint) {
        ...
        return this.sequelize.query(`ALTER TABLE "${table}" ADD CONSTRAINT "${constraint.name}" ${constraint_type} ("${constraint.fields.join('", "')}")`);
    }
    dropConstraint(table, constraint) {
        let queryInterface = this.sequelize.getQueryInterface();
        if (constraint.name) {
            return this.sequelize.query(`ALTER TABLE "${table}" DROP CONSTRAINT IF EXISTS "${constraint.name}"`).then(() => {
                return this.sequelize.getQueryInterface().removeIndex(table, constraint.name);
            });
        }
            ...
            if (constraint.type == "references") {
                return this.getFKs(table).then((fks) => {
                    if (fks[constraint.field]) {
                        return this.sequelize.query(`ALTER TABLE "${table}" DROP CONSTRAINT "${fks[constraint.field].constraint_name}"`);
                    }
                });
            }
            return this.getIndices(table).then((fields) => {
                ...
                for (let index of fields[constraint.field]) {
                    if (index.unique) {
                        ...
                        ((index) => {
                            p = p.then(() => {
                                return this.sequelize.query(`ALTER TABLE "${table}" DROP CONSTRAINT IF EXISTS "${index.name}"`).then(() => {
                                    return queryInterface.removeIndex(table, index.name);
                                });
                            });
                        })(index);
                    }
    ...
    castColumnType(table, column_name, old_type, type) {
        ...
        if (queryCast) {
            return this.sequelize.query(`ALTER TABLE ${table} ALTER COLUMN "${column_name}" TYPE ${type} USING ${queryCast}`).then(() => {
                return true;
            });
        }
        else {

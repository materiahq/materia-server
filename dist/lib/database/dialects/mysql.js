...
class MysqlDialect extends abstract_1.AbstractDialect {
    ...
    addConstraint(table, constraint) {
        ...
        return this.sequelize.query(`ALTER TABLE ${table} ADD ${constraint_sql} (${constraint.fields.join(', ')})`);
    }
    dropConstraint(table, constraint) {
        if (constraint.name) {
            ...
            return this.sequelize.query(`ALTER TABLE ${table} DROP PRIMARY KEY`);
        }
        else if (constraint.field) {
            if (constraint.type == "references") {
                return this.getFKs(table).then((fks) => {
                    if (fks[constraint.field]) {
                        return this.sequelize.query(`ALTER TABLE ${table} DROP FOREIGN KEY ` + fks[constraint.field].constraint_name);
                    }
                });
            }
            return this.getIndices(table).then((fields) => {
                ...
                for (let index of fields[constraint.field]) {
                    if (index.unique) {
                        ...
                        ((index) => {
                            p = p.then(() => {
                                return this.sequelize.query(`ALTER TABLE ${table} DROP INDEX ${index.name}`);
                            });
                        })(index);
                    }
                }

...
class Server {
	constructor(app) {
		...
		this.expressApp.use(express.static(path.join(this.app.path, 'static')));
		//}
	...
	hasStatic() {
		return fs.existsSync(path.join(this.app.path, 'static', 'index.html'))
	}
	...
	start(host, port) {
		let promise = new Promise((resolve, reject) => {
			...
			this.expressApp.all('/*', (req, res) => {
				if (fs.existsSync(path.join(this.app.path, 'static', '404.html'))) {
					res.sendFile(path.join(this.app.path, 'static', '404.html'))
				}
				else if (this.hasStatic()) {
					res.sendFile(path.join(this.app.path, 'static', 'index.html'))
				}
				else {
					res.status(404).send({
						error: true,

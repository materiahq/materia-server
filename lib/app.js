...
module.exports = class App extends events.EventEmitter {
	...
	initializeStaticDirectory() {
		if ( ! fs.existsSync(path.join(this.path, 'static'))) {
			fs.mkdirSync(path.join(this.path, 'static'))
		}
		if ( ! fs.existsSync(path.join(this.path, 'static', 'index.html'))) {
			fs.appendFileSync(path.join(this.path, 'static', 'index.html'), `<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">

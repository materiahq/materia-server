	...
	start() {
		...
		let promise = new Promise((resolve, reject) => {
			...
			auth.then(() => {
				...
				console.log('Impossible to connect the database (error in database.json)')
				console.log('The database has been disabled')
				console.log(this.type, this.host, this.port, this.database, this.username, this.password, this.opts)
				this.disabled = true
				resolve(e)
			})
		})

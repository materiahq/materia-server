...
class Database {
							...
							for (let field of fields) {
								for (let index of indexes) {
									...
									for (let ind of inds) {
										...
										let f = column_names[index.column_indexes.indexOf(parseInt(ind))]
										if (f == field.Field) {
											field.primary = field.primary || index.primary
	...
	_fromdbField(field) {
		...
		if (/nextval\('.+'::regclass\)/.exec(field.Default)) {
			...
			res.default = true
			let textValue = /'(.*)'::text/.exec(field.Default)
			if (textValue)
	...
	_translateField(field) {
		...
		if (field.default != undefined) {
			...
			if (res.default && field.defaultValue != undefined) {

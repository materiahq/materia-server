...
class History {
	...
	load() {
		try {
			let changes = fs.readFileSync(path.join(this.app.path, 'history', 'changes.json'))
	...
	save() {
		this.cleanStacks()
		fs.existsSync(path.join(this.app.path, 'history')) || fs.mkdirSync(path.join(this.app.path, 'history'));
		fs.writeFileSync(path.join(this.app.path, 'history', 'changes.json'), JSON.stringify(this.diff, null,  '\t'))
	}

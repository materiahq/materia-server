...
class DBEntity extends Entity {
	...
	getIndices() {
		let query = this.app.database.sequelize.getQueryInterface().QueryGenerator.showIndexesQuery(this.model.tableName)
				...
				if (field.unique == false) {
					return this.getUniqueIndex(field.name).then((index) => {
						...
						return this.app.database.sequelize.query(
							'ALTER TABLE ' + this.model.tableName + ' DROP CONSTRAINT ' + index.name
						).then(() => {
							return queryInterface.removeIndex(
								this.model.tableName, index.name
							)
						})
					})
				}

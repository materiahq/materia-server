...
class Entity {
	...
	rename(name, ignore_history) {
		for (let entity in this.app.entities.findAll()) {
			if (entity.name == name) {
				return false
			}
		}
	...
	*/
	...
	addFieldAt(field, at, ignore_history) {
		if ( ! field.name) {
			return null
		}
		field.edit = false
	...
	addField(field, ignore_history) {
		return this.addFieldAt(field, this.fields.length, ignore_history)
	}
	
	addFieldFirst(field, ignore_history) {
		return this.addFieldAt(field, 0, ignore_history)
	}

	removeField(name, ignore_history) {
		if (! name) {
			return false
		}

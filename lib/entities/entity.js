...
class Entity {
	...
	create(fields, relations, queries, isRelation, options) {
		options = options || {}
	...
	removeRelation(relation, options) {
		...
		if (relation.type == 'belongsToMany') {
			...
			if (entityThrough && entityThrough.isRelation) {
				...
				p = entityThrough.updateField(relationField.name, relationField, options).then(() => {
					if ( ! relation.paired) {
			...
			p = this.removeField(relation.field, options)

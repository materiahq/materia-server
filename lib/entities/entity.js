...
class Entity {
	...
	create(entityobj, options) {
		...
		this.relations = []
		this.queries = []
		this.createdAt = entityobj.createdAt
		this.updatedAt = entityobj.updatedAt
				...
				entityobj.queries.forEach((query) => {
					...
					if (reservedQueries.indexOf(query.id) == -1) {
						try {
							...
							if (e.originalError) {
								console.log('Skipped query ' + query.id + ' of entity ' + this.name)
								console.log('due to error: ' + e.originalError.stack)
							}
							else {
								throw e
							}
		...
		for (let relobj of this.relations_queue) {
			if (this.app.entities.get(relobj.relation.reference.entity))
				promises.push(this.addRelation(relobj.relation, relobj.options).catch((e) => {
					this.app.warning(`In ${this.name}: ${e && e.message}. Skipped relation`)
	...
	getRelatedEntities() {
		...
		for (let entity of this.app.entities.findAll()) {
			for (let entityRelation of entity.relations) {
				if (entityRelation.reference.entity == this.name) {
					associatedEntityArray.push(entity)
				}
			}
		}
		//To find associatedTable from belongsToMany relation
		/*for (let entity of this.app.entities.findAll()) {
			if (entity.isRelation) {
				for (let entityRelation of entity.isRelation) {
					if (entityRelation.entity == this.name) {
						associatedEntityArray.push(entity)
					}
				}
			}
		...
		for (let relation of this.relations) {
			if (relation.reference && relation.reference.entity) {
				for (let entityRegister of associatedEntityArray) {
					if (entityRegister.name == relation.reference.entity) {
						match = true
					}
				}
				if ( ! match ) {
					associatedEntityArray.push(this.app.entities.get(relation.reference.entity))
				}
			}
		}
	...
	*/
	...
	removeRelation(relation, options) {
		...
		if (options.apply != false) {
			...
			console.log("Relation in server", relation)
			this.app.entities.get(relation.entity).save(options)
			p = this.app.entities.get(relation.reference.entity).removeRelation(inversedRelation, options).catch(() => {
				return Promise.resolve()
			})
		}
	...
	@param {string} - Relation's description.
	@returns {Field}
	*/
	getField(name) {

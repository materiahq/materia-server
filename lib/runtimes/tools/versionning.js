...
class Versionning {
	...
	_diffEntity(entity, dbTable, diffs) {
		...
		for (let _dbField of dbTable) {
			...
			if (this._diffRelation(entity, dbField, field, diffs))
				continue
		...
		for (let relation of entity.getRelations()) {
			if ( ! dbFields[relation.field] && relation.type == 'belongsTo') {
				// delete relation
				diffs.relations.push({
					redo: {
						type: this.DiffType.DELETE_RELATION,
	...
	entitiesToDatabase(diffs) {
		...
		for (let type of ['relations', 'fields', 'entities']) {
			for (let action of diffs[k]) {
				actions.push(entityAction)
			}
		}
		return this.app.history.revert(diffs, {history:false, apply:false, save:false})
	}

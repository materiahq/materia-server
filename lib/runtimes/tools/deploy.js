...
class Deploy {
	...
	generate(provider, _options) {
		...
		return this.checkProviderSetup(provider, options).then(() => {
			...
			if (provider == 'aws') {
				...
				infos.instance.aws_instance_type = options['aws-instance-type']
	...
	spawnRelease(provider, options) {
		if (provider == 'dockerfile') {
			...
			let args = ['docker:release']
			if (options['heroku-app'])
				args.push('--app=' + options['heroku-apps'])
			return cp.spawn(this._binPath['heroku'], args, {cwd: this.app.path})
		} else if (provider == 'aws') {
			fs.chmodSync(this.app.path + '/deploy-aws.sh', '755');
	...
	checkProviderSetup(provider, options) {
		if (provider == 'dockerfile')
			...
			return whereis('docker').then((path) => {
				...
				return new Promise((accept, reject) => {
					cp.exec(this._binPath['heroku'] + ' plugins', (error, stdout, stderr) => {
						...
						if (error || stderr || stdoutstr == '')
							return reject(error)
						if ( ! stdoutstr.match(/\sheroku-docker@/))
							return reject(new Error("heroku-docker plugin not installed"))
						accept()
					})
				})
			}).then(() => {
				...
				return new Promise((accept, reject) => {
					fs.readFile(homedir + '/.netrc', (err, data) => {
				...
				if (options['heroku-app'])
					return Promise.resolve({})
				return new Promise((accept, reject) => {
					fs.readFile(this.app.path + '/.git/config', (err, data) => {

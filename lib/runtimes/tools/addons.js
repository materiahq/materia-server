...
class AddonsTools {
	constructor(app) {
		...
		this._templates_dir = this.app.materia_path + "/../templates/addons/"
	}

	install(name, options) {
		let content = fs.readFileSync(this._templates_dir + 'install.sh.hbs').toString()
		let tmpl = Handlebars.compile(content)

		let temp_root = process.env['HOME'] || '/tmp'

		let infos = {
			temp_dir: temp_root + '/.tmp_addon',
			temp_package: this._templates_dir + 'package.json',
			addon_package: name,
			addons_dir: this.app.path + '/addons'
		}
		...
		fs.writeFileSync(infos.temp_dir + '/install.sh', content)
		fs.chmodSync(infos.temp_dir + '/install.sh', "755")

...
class Api {
	constructor(app) {
		...
		this.app.history.register(this.DiffType.ADD_API_PARAM, (data, opts) => {
			return this.get(data.method, data.url).addParam(data.value, opts)
		})

		this.app.history.register(this.DiffType.DELETE_API_PARAM, (data, opts) => {
			return this.get(data.method, data.url).delParam(data.value, opts)
		})

		this.app.history.register(this.DiffType.ADD_API_DATA, (data, opts) => {
			return this.get(data.method, data.url).addData(data.value, opts)
		})

		this.app.history.register(this.DiffType.DELETE_API_DATA, (data, opts) => {
			return this.get(data.method, data.url).delData(data.value, opts)
		})
	}

...
class Api {
	...
	*/
	...
	load() {
		...
		let content
		try {
			content = fs.readFileSync(path.join(this.app.path, 'api.json'))
		}
		catch(e) {
			return
		}
		...
		try {
			...
			endpoints.forEach((endpoint) => {
				try {
					...
					this.app.warn('Skipped endpoint ' + endpoint.method + ' ' + endpoint.url)
					this.app.warn('due to error ' + e.message)
				}
			})
		} catch (e) {
			if (e.code != 'ENOENT')

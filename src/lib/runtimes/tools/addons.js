...
class AddonsTools {
	...
	parse_package(name) {
		...
		else if (( matches = /^(.+@[\w\d\.]+:(?:.*\/))(.*?)(?:\.git)?(?:#(.*))?$/.exec(name) )) {
			// type git remote url
			pkg.git = matches[1]
			pkg.name = matches[2]
			pkg.version = matches[3]
			pkg.vop = "#"
		}
	...
	_prepare_temp(pkg, script) {
		...
		//TODO: need to verify the path of bash (whereis)
	...
	install(name, opts) {
		...
		proc.on('close', (code) => {
			...
			//TODO: call afterInstall() static function of the addon class if it exists
		})
		return proc
	}
	...
	remove(name, opts) {
		...
		return new Promise((accept, reject) => {
			...
			remove(path.join(this.app.path, 'addons', pkg.name), (err) => {
				...
				this.app.addons.unload(pkg.name)
				this.app.saveMateria(opts)
				//TODO: call the uninstall() static function of the addon class if it exists
	...
	search(query) {
		return new Promise((accept, reject) => {
			request({
				url: 'http://getmateria.com/api/addon/search',
				qs: {q: query},
				json: true
			}, (error, response, body) => {
